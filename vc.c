...
IVC *vc_read_image(char *filename) {
  ...
  if ((file = fopen(filename, "rb")) != NULL) {
    ...
    if (strcmp(tok, "P4") == 0) {
      ...
      printf("ERROR -> vc_read_image():\n\tFile is not a valid PBM, PGM or PPM "
             "file.\n\tBad magic number!\n");
    ...
    if (levels == 1) // PBM
    {
      if (sscanf(netpbm_get_token(file, tok, sizeof(tok)), "%d", &width) != 1 ||
          sscanf(netpbm_get_token(file, tok, sizeof(tok)), "%d", &height) !=
              1) {
      ...
      sizeofbinarydata =
          (image->width / 8 + ((image->width % 8) ? 1 : 0)) * image->height;
      ...
      if ((v = fread(tmp, sizeof(unsigned char), sizeofbinarydata, file)) !=
          sizeofbinarydata) {
    ...
    } else // PGM ou PPM
    {
      if (sscanf(netpbm_get_token(file, tok, sizeof(tok)), "%d", &width) != 1 ||
          sscanf(netpbm_get_token(file, tok, sizeof(tok)), "%d", &height) !=
              1 ||
          sscanf(netpbm_get_token(file, tok, sizeof(tok)), "%d", &levels) !=
              1 ||
          levels <= 0 || levels > 255) {
        printf("ERROR -> vc_read_image():\n\tFile is not a valid PGM or PPM "
               "file.\n\tBad size!\n");
...
int vc_write_image(char *filename, IVC *image) {
  ...
  if ((file = fopen(filename, "wb")) != NULL) {
    if (image->levels == 1) {
      sizeofbinarydata =
          (image->width / 8 + ((image->width % 8) ? 1 : 0)) * image->height + 1;
      tmp = (unsigned char *)malloc(sizeofbinarydata);
      if (tmp == NULL)
        return 0;
      ...
      totalbytes =
          unsigned_char_to_bit(image->data, tmp, image->width, image->height);
      printf("Total = %ld\n", totalbytes);
